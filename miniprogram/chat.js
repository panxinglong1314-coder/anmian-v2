        ...
        if (!res.authSetting['scope.record']) {
          wx.authorize({
            ...
            fail: () => {
              ...
              wx.showModal({
                title: '需要录音权限',
                content: '知眠需要通过语音与您对话，请在设置中开启录音权限',
                confirmText: '去设置',
                success: (modalRes) => {
                  if (modalRes.confirm) {
                    wx.openSetting()
...
准备好入睡了。晚安。`
  ...
  _registerRecorderEvents() {
    ...
    recorderManager.onError((err) => {
      ...
      // 致命错误：隐私协议未声明、权限未授权等，不应无限重试
      const isFatal = err.errMsg && (
        err.errMsg.includes('privacy agreement') ||
        err.errMsg.includes('scope') ||
        err.errMsg.includes('not declared') ||
        err.errMsg.includes('authorize') ||
        err.errMsg.includes('denied') ||
        err.errMsg.includes('not authorized')
      )
      if (isFatal) {
        wx.showToast({ title: '录音权限未开启或隐私协议未配置', icon: 'none', duration: 3000 })
        this.exitSleepMode()
        return
      }
      if (this.data.sleepModeActive) {
        ...
        // 非致命错误延迟重试，避免死循环
        if (this._listenTimer) clearTimeout(this._listenTimer)
        this._listenTimer = setTimeout(() => this._startVADLoop(), 1200)
      }
    })
