...
修复：数据源从本地文件 → evaluation_tracking文件(AI质量) + Redis(会话统计)
保持原有返回格式，兼容前端

修改日志 2026-05-11:
- 修复 night_ratio 硬编码为0的bug（按会话开始时间计算22:00-06:00占比）
- 修复 last_seen 不更新的bug（改为取会话最后一条消息时间）
- 修复 first_seen 为空的问题
"""
import json
import os
...
def _get_all_sessions(days: int = 30) -> List[Dict]:
    ...
    for key in history_keys:
        ...
        try:
            ...
            first_ts = session_data[0].get("timestamp", "") if session_data else ""
            last_ts = session_data[-1].get("timestamp", "") if session_data else first_ts

            # 过滤日期范围
            if first_ts:
                try:
                    st = datetime.fromisoformat(first_ts.replace("Z", "+00:00")).replace(tzinfo=None)
                    if st < cutoff:
                        continue
                except Exception:
                    pass
...
def get_dashboard_stats(days: int = 7, limit: int = 500) -> Dict[str, Any]:
    """仪表盘核心指标（保持原有格式，数据来自evaluation_tracking）"""
    ...
    # ===== 修复：夜间对话占比计算 =====
    # 统计 session 开始时间在 22:00-06:00 之间的比例
    night_count = 0
    for s in sessions:
        start_ts = s.get("start_time", "")
        if start_ts:
            try:
                st = datetime.fromisoformat(start_ts.replace("Z", "+00:00")).replace(tzinfo=None)
                if _is_night_time(st):
                    night_count += 1
            except Exception:
                pass
    ...
    return {
        ...
        "avg_duration_min": 0,
        "night_ratio": night_ratio,   # ✅ 修复：不再硬编码0
        "night_count": night_count,  # 新增：夜间会话绝对数量
...
def get_quality_stats(days: int = 30, limit: int = 500) -> Dict[str, Any]:
    """AI 质量监控统计（基于 evaluation_tracking，保持原格式）"""
    eval_records = _load_evaluation_records(days=days)
    if not eval_records:
        return {"message": "暂无数据"}
    ...
    for s in sessions:
        ...
        if uid not in users:
            users[uid] = {
                "user_id": uid,
                "first_seen": start_ts,
                "last_seen": last_ts,       # ✅ 修复：用 last_time 而非 start_time
                "session_count": 0,
                "avg_rating": None,
