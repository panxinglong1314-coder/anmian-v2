...
async def asr_websocket(websocket: WebSocket):
    ...
    print(f"[ASR-WS] enter receive loop, pcm_buffer size: {len(pcm_buffer)}")
    try:
        while True:
            data = await websocket.receive()
            msg_keys = list(data.keys())
            print(f"[ASR-WS] received msg keys: {msg_keys}")
            if "bytes" in data:
                pcm = data["bytes"]
                ...
                if pcm_len > 0:
                    ...
                    frame_count += 1
                    print(f"[ASR-WS] binary frame #{frame_count}: {pcm_len}B total:{len(pcm_buffer)}")
            elif "text" in data:
                text_data = data["text"]
                ctrl = json.loads(text_data)
                print(f"[ASR-WS] text msg: {ctrl}")
