...
class Settings(BaseSettings):
    ...
    # ✅ 多账号负载均衡（备用账号，格式：TENCENTCLOUD_APP_ID_2, TENCENTCLOUD_SECRET_ID_2, TENCENTCLOUD_SECRET_KEY_2）
    ...
    # Redis 异步客户端（高并发优化，替代同步 redis_client）
    # 环境变量: REDIS_ASYNC_URL，格式: redis://:password@host:port/db
    redis_async_url: str = ""

    # TTS 高频短句预热（启动时用 Edge TTS 预合成，免费不占并发）
    # 环境变量: TTS_WARMUP_PHRASES，逗号分隔，如："嗯,我在,好的,嗯嗯,稍等,我在这儿"
    tts_warmup_phrases: str = "嗯,我在,好的,稍等,嗯嗯,我在听,好,嗯嗯嗯",

    # ASR 预热连接数（启动时预先建立 ASR WS 连接，减少首次延迟）
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if settings.redis_async_url:
            async_redis_client = await redis.asyncio.from_url(
