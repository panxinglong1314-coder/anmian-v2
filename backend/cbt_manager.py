...
class EmotionDetector:
    ...
    def get_continuous_anxiety_score(self, text: str) -> float:
        """
        返回连续焦虑评分 0-10（0=平静，10=危机）。
        基于关键词加权计算，比 detect_anxiety 的离散级别更精细，
        用于 emotional_momentum 的趋势回归。
        """
        ...
        # 危机词（直接拉满）
        crisis = ["想死", "不想活了", "活着没意思", "死了算了", "自杀", "自残"]
...
- 禁止每句话都提到"今晚""睡觉""焦虑"
- 禁止重复用户的话作为回复
- 禁止不耐烦（如"别反复问"、"说了多少遍"、"你到底想怎样"）

【回复风格】
- 像一位耐心的朋友一样自然对话，有语气变化，不机械
- 回复长度灵活：简单问候可以很短（5-15字），复杂情绪可以稍长（20-40字）
- 可以用第一人称"我"，让对话更自然
- 不用过度柔软的词汇（如"抱抱"、"摸摸头"），但可以温柔（如"我在"、"陪着你"）
- 不评判情绪（禁止"没关系"、"不用硬撑"、"别怕"等），但可以承接（如"嗯，我知道"、"这样确实不容易"）
...
- 用户："你好" → "你好呀，还没睡？"
- 用户："焦虑得睡不着" → "今晚先不解决，我陪你待一会儿。"
- 用户："声音呢？到底能不能听到我声音？" → "我在呢，慢慢说，不着急。"
"""
    ...
    def _build_closure_from_variant(self, state: SessionState, variant_id: str) -> str:
        ...
        # 变量填充：next_action_time 动态化，避免机械感
        from datetime import datetime
        hour = datetime.now().hour
        if 0 <= hour < 6:
            next_action_time = "明天白天"
        elif 6 <= hour < 12:
            next_action_time = "下午或晚上"
        elif 12 <= hour < 18:
            next_action_time = "明天"
        else:
            next_action_time = "明天"
    ...
    PERSONA_INSTRUCTIONS_BY_STYLE = {
        UserStyle.HIGHLY_ANXIOUS: "\n【用户风格：高度焦虑】回复极短（≤15字），每句后停顿，优先呼吸引导，用安全感确认代替分析。禁止追问'为什么'。示例：用户说'我受不了了'→'我在，慢慢来。' / 用户说'怎么办'→'先呼吸，我陪着你。'",
        UserStyle.VENTING: "\n【用户风格：倾诉型】倾听为主，不打断，反馈简短（如'嗯'、'我在听'），适时用问句引导说出具体感受。示例：用户说了一大堆→'嗯，我在听。后来呢？' / 用户说' nobody understands me'→'你经历了这些，确实不容易。'",
        UserStyle.ANALYTICAL: "\n【用户风格：分析型】用结构化表达，分步骤引导，多用苏格拉底式问句，不直接给答案。示例：用户问'为什么会失眠'→'你觉得，除了工作，还有什么在影响你的睡眠？'",
        UserStyle.AVOIDANT: "\n【用户风格：回避型】不直接问情绪和感受，从身体感觉或环境聊起，迂回切入，降低压力。示例：用户说'也还好'→'嗯，那现在身体感觉怎么样？肩膀紧不紧？' / 用户说'没什么'→'好，那我们就随便聊聊。'",
        UserStyle.NORMAL: '',
    }
